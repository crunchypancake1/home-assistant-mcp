...
- **`src/ha/api.ts`** — `HomeAssistantClient`: thin wrapper around the HA REST API. Handles auth (Bearer token), path-safety (`encodeURIComponent`), and response error normalisation. `listAreas()` uses the `/api/template` endpoint with a Jinja template since HA has no direct areas REST endpoint.
- **`src/types.ts`** — Shared types: `Env` (Worker bindings + vars), `HaState`, `HaArea`.
...
- Custom domain: `home-assistant-mcp.crunchypancake.com`.
...
Both `.js` and `.ts` source files exist side-by-side (`src/agent.js`, `src/agent.ts`, etc.) — the `.ts` files are the canonical source; the `.js` files appear to be build artefacts or leftovers and can be ignored.
